...
ColorConversion::Rgb::Rgb(double r, double g, double b) : r{ r }, g{ g }, b{ b }
{
}

ColorConversion::Hsv::Hsv(double h, double s, double v) : h{ h }, s{ s }, v{ v }
{
}

std::optional<unsigned long> ColorConversion::TryParseInt(const std::wstring_view& s)
{
    return TryParseInt(s, 10 /* base */);
}

std::optional<unsigned long> ColorConversion::TryParseInt(const std::wstring_view& str, int base)
{
...
ColorConversion::Hsv ColorConversion::RgbToHsv(const Rgb& rgb)
{
    double hue = 0;
    double saturation = 0;
    double value = 0;
...
ColorConversion::Rgb ColorConversion::HsvToRgb(const ColorConversion::Hsv& hsv)
{
    double hue = hsv.h;
    ...
    if (chroma == 0)
    {
        return ColorConversion::Rgb(min, min, min);
    }
    ...
    return ColorConversion::Rgb(r, g, b);
}

ColorConversion::Rgb ColorConversion::HexToRgb(const std::wstring_view& input)
{
    const auto [rgb, a] = HexToRgba(input);
    return rgb;
}

winrt::hstring ColorConversion::RgbToHex(const Rgb& rgb)
{
...
std::tuple<ColorConversion::Rgb, double> ColorConversion::HexToRgba(const std::wstring_view& input)
{
...
winrt::hstring ColorConversion::RgbaToHex(const ColorConversion::Rgb& rgb, double alpha)
{
...
wu::Color ColorConversion::ColorFromRgba(const ColorConversion::Rgb& rgb, double alpha)
{
...
ColorConversion::Rgb ColorConversion::RgbFromColor(const wu::Color& color)
{
