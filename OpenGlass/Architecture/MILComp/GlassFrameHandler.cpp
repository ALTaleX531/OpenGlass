...
namespace OpenGlass::GlassFrameHandler
{
	...
	extern "C" bool MyCLegacyNonClientBackground_HasSomethingToRender(uDWM::CLegacyNonClientBackground* This);
	extern "C" bool MyCLegacyNonClientBackground_HasSomethingToRender_Impl(uDWM::CLegacyNonClientBackground* This);
...
void GlassFrameHandler::MyCRectangleVisual_SetRect(uDWM::CRectangleVisual* This, const D2D1_RECT_F& rc)
{
	const auto window = uDWM::TryGetWindowFromVisual(This);
	if (
		!window ||
	...
	if (const auto hr = reflectionVisual->UpdateSurface(GlassKernel::GetOrCreateReflectionSurface()); FAILED(hr))
	{
		LOG_HR(hr);
		return;
	}
	if (
		const auto hr = reflectionVisual->UpdateViewport(
			reflectionVisual->GetLocalToParentVisualOffset(window->GetTransformParent()),
			Shared::g_reflectionParallaxIntensity,
			window->IsRTLMirrored(),
			size.cx,
	...
	if (
		const auto hr = reflectionVisual->UpdateOpacity(
			Shared::g_reflectionPolicy & Shared::ReflectionPolicy::NonClient ?
			GlassKernel::GetAdjustedReflectionIntensity(
				window->TreatAsActiveWindow(),
				window->TreatAsMaximized()
...
int GlassFrameHandler::MyCTopLevelWindow_EdgeBorderMustBeOpaque(uDWM::CTopLevelWindow* This)
{
