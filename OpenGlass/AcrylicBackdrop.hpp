...
namespace OpenGlass::AcrylicBackdrop
{
	double GetTintOpacityModifier(wu::Color tintColor)
	{
		...
		const auto rgb{ ColorConversion::RgbFromColor(tintColor) };
		const auto hsv{ ColorConversion::RgbToHsv(rgb) };
...
#pragma pop_macro("max")
	...
	{
		const ColorConversion::Rgb rgbTintColor = ColorConversion::RgbFromColor(tintColor);

		// If luminosity opacity is specified, just use the values as is
		if (luminosityOpacity)
		{
			return ColorConversion::ColorFromRgba(rgbTintColor, std::clamp(luminosityOpacity.value(), 0.0, 1.0));
		}
		else
		{
			...
			const ColorConversion::Hsv hsvTintColor = ColorConversion::RgbToHsv(rgbTintColor);

			const auto clampedHsvV = std::clamp(hsvTintColor.v, minHsvV, maxHsvV);

			const ColorConversion::Hsv hsvLuminosityColor = ColorConversion::Hsv(hsvTintColor.h, hsvTintColor.s, clampedHsvV);
			const ColorConversion::Rgb rgbLuminosityColor = ColorConversion::HsvToRgb(hsvLuminosityColor);
...
#undef min
			return ColorConversion::ColorFromRgba(rgbLuminosityColor, std::min(mappedTintOpacity, 1.0));
#pragma pop_macro("min")
	...
	)
	{
		...
		auto noiseBorderEffect{ winrt::make_self<Win2D::BorderEffect>() };
		...
		auto noiseOpacityEffect{ winrt::make_self<Win2D::OpacityEffect>() };
		...
		auto blendEffectOuter{ winrt::make_self<Win2D::BlendEffect>() };
