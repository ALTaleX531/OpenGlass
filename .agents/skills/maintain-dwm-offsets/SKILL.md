...
4. For a Symbol, run `python .agents/skills/maintain-dwm-offsets/scripts/audit_symbol_resolution.py . --architecture legacy|milcomp --module udwm|dwmcore --version BUILD.REVISION --image PATH_TO_DLL --symbol-path PATH_TO_SYMBOLS [--configuration release|debug] [--dbghelp PATH_TO_DBGHELP] [--id STABLE_ID]`. The default configuration is Release, so Debug-only descriptors remain inactive. Record the selected DbgHelp path, version, and hash plus the PDB identity and hash because exact `UNDNAME_COMPLETE` text is resolver-version input. Only a CodeView GUID/age-matched PDB and matching PE version is production name evidence. An unpaired PDB is discovery-only. Discovery can still return a successful process exit when the requested names resolve; automation making a production claim must require `evidence: production_candidate` and `pdb.paired: true`, not merely exit code zero.
   ...
   To lint every indexed x64 revision of a Windows build, run `python .agents/skills/maintain-dwm-offsets/scripts/audit_winbindex_revisions.py . --architecture legacy|milcomp --module udwm|dwmcore --build BUILD --output REPORT.json`. This uses Winbindex only to enumerate image identities, downloads images and exact PDBs from Microsoft's public symbol server, verifies image SHA-256/version and PDB GUID/age, then applies the single-sample audit to every unique image. Active Optional descriptors are failures by default so the report can justify precise version ranges and eventual removal of Optional; use `--allow-optional-missing` only for an explicitly exploratory run. `--list-only` inventories revisions without downloading binaries, and `--offline` makes the cache a hard prerequisite. A passing batch report proves exact symbol-name resolution and the declared function-pointer ABI's name-level applicability across those samples; it does not prove semantics, calling convention implementation, object layout, instruction-pattern stability, or runtime behavior without the corresponding IDA and real-OS evidence.
...
- **Schema selection**: architecture and requested build/revision; for a Layout, the selected schema ID, entry and interval or `unsupported`; for a Symbol, the active range, status, matched complete name and RVA. Use `not applicable` when the audited item has no corresponding selection.
