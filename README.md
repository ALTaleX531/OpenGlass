...
> [!NOTE]
> 1. OpenGlass does not support HKLM\SOFTWARE\Microsoft\Windows\DWM
...
| DisabledHooks | DWORD | **Undocumented** |  |  | **0x01 = Disable the hooks of CaptionTextHandler.cpp to enable compatibility with third-party mods** |
...
| ColorizationGlassReflectionIntensity | DWORD | The intensity of reflection effect (0-100%). Default value is 0%. |  |  | **OK** |
|  | DWORD | **Not supported** | ColorizationGlassReflectionParallaxIntensity | The parallax intensity of the refection effect (0-100%). Default value is 10%. |  |
| CustomThemeReflection | String | path to PNG file which will be used as overlay image to simulate reflection (Aero stripes) effect |  |  | **OK** |
...
| TextGlowMode | DWORD | Specifies how window caption glow effect will be rendered. 0x0=No glow effect. 0x1=Glow effect loaded from atlas (default). 0x2=Glow effect loaded from atlas and theme opacity is respected. 0x3=Composited glow effect using your theme settings. HIWORD of the value specifies glow size (0 = theme default). | TextGlowSize | The value specifies glow size. Default value is 15. | **TextGlowMode is deprecated, use TextGlowSize instead** |
...
|  | DWORD | **Not supported** | GlassType | The type of backdrop effect (0x0-0x4). |  |
|  | DWORD | **Not supported** | GlassOverrideBorder | Specifies that the effect should extend to the border. |  |
|  | DWORD | **Not supported** | EnableGlassRegionSplitting | Split the backdrop region to improve performance. |  |
|  | DWORD | **Not supported** | GlassCrossFadeTime | The cross fade time for backdrop switching. |  |
|  | DWORD | **Not supported** | GlassOverrideAccent | Overriding accent with the effect of OpenGlass. |  |
|  | DWORD | **Not supported** | EnableOcclusionCulling | Enable occlusion cullling optimization for Windows 10. |  |
