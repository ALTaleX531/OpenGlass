...
- Keep the injected DLL's registry lookup independent from the GUI's recommended write-location policy. Every named runtime value accepts `HKCU → HKLM → default`; for an overridable colorization property, each hive's Override name precedes its base name, producing `HKCU Override → HKCU base → HKLM Override → HKLM base → default`. This is a long-term compatibility contract for conversion packs and plug-ins, not a temporary migration fallback. The GUI's HKCU/HKLM placement is a product convention, not a runtime validity rule. Recommended-location enforcement applies only to writes by the official GUI and preset packages. The GUI scans for values outside its recommended location on every startup, stays silent when none exist, and performs a transactional two-hive migration only after confirmation.
